...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
    ...
    sum_h=prev_h.dot(Wh) + x.dot(Wx) + b
...
def rnn_step_backward(dnext_h, cache):
    ...
    dx = np.dot(dnext_h, Wx.T) #근데 Wx 왜 Transpose 해야하는걸까
    dprev_h = np.dot(dnext_h, Wh.T)
    #dWh=np.dot(prev_h, dnext_h.T)
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    do = dnext_h * np.tanh(next_c) * sigmoid_grad(o)
    dnext_c = dnext_c + dnext_h * o * (1 - np.tanh(next_c)**2)
    dprev_c = dnext_c * f
    df = dnext_c * prev_c * sigmoid_grad(f)
