...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    """
    #미리 설정해둔값있으면 그거 가져오고 아니면 오른쪽 default 값
    mode = bn_param["mode"]
    ...
    if mode == "train":
        ...
        batch_mean = np.mean(x,axis=0) #왜 axis 0임
        batch_var = np.var(x, axis = 0)
        #running mean/var은 이따가 test할때만 사용됨
        #근데 왜 이렇게 하는건지 솔직히 모르겠어 이유를알고싶어
        ...
        #cache = [gamma, x_hat, beta, 1 / (np.sqrt(batch_var) + eps)]
...
def batchnorm_backward_alt(dout, cache):
    ...
    ###########################################################################
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    x,batch_mean,batch_var,x_std,eps,gamma,beta = cache
    
  
    N = x.shape[0]
    # Calculate the change factor first, and calculate it better
    dgamma = np.sum(dout * x_std, axis = 0)
    dbeta = np.sum(dout, axis = 0)
    # Then calculate the gradient to x
    a = 1 / np.sqrt(batch_var + eps)
    dx_hat = dout * gamma

    dvar = np.sum(dx_hat * (x - batch_mean) * (-0.5) * (a ** 3), axis = 0)
    dmean = np.sum(- dx_hat * a, axis = 0) #+ dvar * (-2 / N) * np.sum(x-batch_mean, axis = 0) #The latter item is 0
